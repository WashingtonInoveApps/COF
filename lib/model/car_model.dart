...
class CarModel {
  ...
  factory CarModel.fromMapResume(Map<String, dynamic> map) {
    return CarModel(
      id: map['id'] ?? '',
      prefix: map['prefix'] ?? '',
      itens: List<ItensChangesModel>.from(map['itens']?.map((x) => ItensChangesModel.fromMap(x))),
      changes: List<CarChangeModel>.from(map['changes']?.map((x) => CarChangeModel.fromMap(x))),
      status: [],
    );
  }
  ...
  factory CarModel.fromMap(Map<String, dynamic> map) {
    return CarModel(
      ...
      itens: List<ItensChangesModel>.from(map['itens']?.map((x) => ItensChangesModel.fromMap(x))),
      changes: List<CarChangeModel>.from(map['changes']?.map((x) => CarChangeModel.fromMap(x))),
      mapas: List<CarMapaModel>.from(map['mapas']?.map((x) => CarMapaModel.fromMap(x))),
      status: [],
      typeCar: map['typeCar'] ?? '',
      adm: map['adm'] ?? false,
